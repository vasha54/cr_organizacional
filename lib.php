 ...
 * @return string
 */
function theme_cr_organizacional_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_cr_organizacional', 'preset', 0, '/', $filename))) {
    ...
    // cr_organizacional scss.
    $cr_organizacionalvariables = file_get_contents($CFG->dirroot . '/theme/cr_organizacional/scss/cr_organizacional/_variables.scss');
    $cr_organizacional = file_get_contents($CFG->dirroot . '/theme/cr_organizacional/scss/default.scss');
    $security = file_get_contents($CFG->dirroot . '/theme/cr_organizacional/scss/cr_organizacional/_security.scss');

    // Combine them together.
    $allscss = $cr_organizacionalvariables . "\n" . $scss . "\n" . $cr_organizacional . "\n" . $security;
 ...
 * @return string
 */
function theme_cr_organizacional_get_extra_scss($theme) {
    $content = '';
    ...
    if (empty($loginbgimgurl)) {
        $loginbgimgurl = new \moodle_url('/theme/cr_organizacional/pix/loginbg.png');
        $loginbgimgurl->out();
    }
 ...
 * @return string
 */
function theme_cr_organizacional_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 */
function theme_cr_organizacional_get_precompiled_css() {
    global $CFG;

    return file_get_contents($CFG->dirroot . '/theme/cr_organizacional/style/moodle.css');
}
 ...
 * @return mixed
 */
function theme_cr_organizacional_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('cr_organizacional');
