 ...
 * @return bool Success.
 */
function xmldb_theme_cr_organizacional_upgrade($oldversion = 0) {
    global $DB;
    ...
    if ($oldversion < 2022052800) {
        ...
        if ($usertours) {
            foreach ($usertours as $usertour) {
                ...
                if (in_array('boost', $configdata->filtervalues->theme)) {
                    $configdata->filtervalues->theme[] = 'cr_organizacional';
                }
        ...
        upgrade_plugin_savepoint(true, 2022052800, 'theme', 'cr_organizacional');
    }
